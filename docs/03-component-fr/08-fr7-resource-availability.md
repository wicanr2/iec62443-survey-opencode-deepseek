...
FR7 回答「系統還能繼續運作嗎」——抗 DoS、資源管理、故障安全 (fail-safe)、電源中斷恢復、冗餘備援。這是 OT 與 IT 安全最大的分野：在 IT，可用性是 SLA 問題；在 OT，失去可用性本身就是最嚴重的安全後果。
...
故障安全狀態 = 當系統出錯時，設備必須回到一個保證不會傷人的狀態。
