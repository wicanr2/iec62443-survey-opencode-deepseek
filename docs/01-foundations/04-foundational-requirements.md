...
FR (Foundational Requirements) 是 IEC 62443 的骨架——把「工控系統安全」這個模糊的概念拆成七個具體的面向。本篇從根本問題推導：為什麼是這七個？它們各自回答什麼問題？ 不背條文，先重建它們存在的必然性。
...
根本問題是，一指令下來——「緊急停止」「改參數」「刪除任務」——系統必須知道這指令是誰發的，以及這個人/程式/裝置是不是他宣稱的那個人。
...
根本問題是，鑑別完「你是誰」之後，下一件事是「你能做什麼」。一個實習生不該有權停掉全廠的 PLC。
...
根本問題是，你怎麼知道現在執行的 firmware 是原廠給的，還是被注入後門的版本？你怎麼知道 PLC 的參數表沒被偷改一個位元？
...
根本問題是，控制網路上的通訊，有沒有被第三者竊聽？儲存在設備裡的配方、參數、憑證，有沒有被未授權者讀取？
...
根本問題是，在一個工廠網路裡，每個設備該允許跟誰通訊？一台溫度感測器不該直接連到 Internet；一台 PLC 不該能對 MES 發起連線；在 Zone A（控制區）的設備不該能隨意存取 Zone B（管理區）的資料。
...
根本問題是，駭客在你系統裡已經待了三週，你從哪知道？安全事件（登入失敗、權限變更、異常流量）如果沒有被記錄、匯總、告警，等於沒發生過。
...
根本問題是，被 DoS 攻擊打掛了？資源（CPU/memory/storage/bandwidth）被耗盡了？斷電之後回來，系統能自動恢復到安全狀態嗎？
...
- fail-safe：系統壞掉時讓設備回到不會傷人的狀態（閥門自動關閉、馬達停止）
- fail-secure：系統壞掉時維持存取控制不變（門鎖上、防火牆繼續 deny）
