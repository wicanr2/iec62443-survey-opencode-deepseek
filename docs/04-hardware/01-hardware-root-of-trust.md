...
> 一句話定位：硬體信任根 (Hardware Root of Trust) 是 FR3 (SI) 的基石——回答「信任從哪裡開始」。如果最底層的 boot code 可能被篡改，它驗證的 firmware 簽章就沒有任何意義。本篇從根本問題推導信任鏈的設計原則與實作選型。
>
> 前置：[FR 3 (SI)：系統完整性](../03-component-fr/04-fr3-system-integrity.md)
...
```
Boot ROM:
  ├── Immutable boot code（不可修改的 boot 邏輯）
  └── Public key hash（SHA-256 of root public key）

OTP / eFuse:
  └── 存 public key hash（不存 public key 本身，省空間）

Bootloader (flash):
  └── 存 root public key（用 OTP 中的 hash 驗證它沒被改）
```
...
> [物理防篡改與除錯埠管理](02-physical-tamper-protection.md)
