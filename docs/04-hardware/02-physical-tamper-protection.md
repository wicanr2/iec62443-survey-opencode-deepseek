...
> 一句話定位：軟體安全做好了（FR1-7），但攻擊者如果物理接觸你的設備——打開機殼、接上 JTAG、拉出 I2C bus——軟體防禦全部失效。本篇回答：如何偵測物理入侵、如何回應、如何管理除錯介面的生命週期。
>
> 前置：[硬體信任根與 Secure Boot 鏈](01-hardware-root-of-trust.md)
...
| **導電網格 (Tamper Mesh)** | 在 PCB 或機殼內層舖導電線路網格，若被鑽孔或切割→斷路觸發。FIPS 140-2 Level 3+ 要求 | 中 |
...
> [HSM / TPM / Secure Element 選用指南](03-hsm-secure-element.md)
